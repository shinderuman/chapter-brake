...
- Local Web App Serverから継承したライフタイムFDを監視する。
- FDのEOFは親ホストの異常終了として扱い、現在ジョブの即時中断とWebバックエンド
  終了を調停する。通常の終了シグナルによる現在ジョブ完了後停止とは区別する。
- 起動処理中のFDのEOFまたは終了シグナルは、advisory lock待機と外部ツール検査を
  キャンセルし、Unixソケットを作成せず終了する。
...
- 通常の単一バックエンド管理と孤立検出はLocal Web App Serverが所有し、ここでは
  PID探索やソケット所有者判定を行わない。ロックはデータ競合の最終防護に限定する。
- ロック待機は起動コンテキストに従い、親ホスト消滅後に旧ロックの解放を待ってから
  起動処理を再開してはならない。
