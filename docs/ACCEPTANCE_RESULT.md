...
## 2026-08-01 Web Stage 5 結果

- ChapterBrakeと汎用Local Web App Serverの通常・race・vet・build試験は成功した。
- インストール版を`0.0.0.0:8766`で起動し、`lsof`の`*:8766`待受、ループバックAPI、
  LANアドレス`192.168.50.10`経由のstatus/settings APIを確認した。
- インストール版からfaviconとSortableJSが想定パス、MIME typeで配信された。
- 設定PUTは実運用中の値と同じ内容で実行し、値を変更せず原子的保存経路を確認した。
- 実Chromeでインストール版の初期画面、設定モーダル、設定保存を確認した。
- 偽API版でジョブ詳細、一時停止・再開、待機ジョブ削除、SortableJSによるドラッグ並び替えを
  操作し、表示とAPI状態の反映を確認した。
- Web Stage 5の判定は`PASS`とする。
- 実行中キュー追加後に判明した進捗表示のCSP違反と、SSE更新による詳細ボタン・閉じる
  ボタンの差し替え競合を修正した。実Chromeの実行中キューで再現操作を行い、詳細の開閉、
  標準`progress`更新、console errorなしを確認した。
