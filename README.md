...
`/Volumes/2TB HDD/mp4/`、初期区切り時間は`23:40`です。初期版に設定画面は
...
変更できます。
version 1・2の既存設定は、既存値を維持したままversion 3へ自動移行します。
...
4. 出力ベース名と開始番号を確認する。
5. 区切り時間を確認・変更し、近似チェックを調整して出力開始チャプターを選ぶ。
...
- チャプター画面の区切り時間入力中の`Enter`: 入力値で近似チェックを再計算
- `←` / `Backspace`: 前の画面へ戻る
